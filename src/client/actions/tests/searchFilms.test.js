...
test('searchFilms returns empty result without fetching actual data if query is empty', () => {
  const store = mockStore({ searchResults: [] });

  store.dispatch(actions.searchFilms('', 'movie'));
  expect(store.getActions()).toEqual(expectedEmptyAction);
});

test('searchFilms returns empty result if fetching request failed', () => {
  const store = mockStore({ searchResults: [] });
  ...
  store.dispatch(actions.searchFilms('Test', 'tv')).then(() => {
    expect(store.getActions()).toEqual(expectedEmptyAction);
  });
});

test('searchFilms returns search results', () => {
  const store = mockStore({ searchResults: [] });
